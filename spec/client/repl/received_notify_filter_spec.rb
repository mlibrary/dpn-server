...
describe Client::Repl::ReceivedNotifyFilter do
  ...
  it "includes fixity_complete" do
  ...
  it "ignores received_notify_ongoing" do
    flow = Fabricate(:received_notify_ongoing_replication_flow)
    expect(filter.flows).to_not include(flow)
  end
  it "ignores flows with ongoing AND failed" do
    flow = Fabricate(:received_notify_ongoing_replication_flow)
    flow.received_notify_attempts.create!(start_time: 1.day.ago, end_time: 1.hour.ago, success: false)
    expect(filter.flows).to_not include(flow)
  end
end
